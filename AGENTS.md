...
`ep_real_time_chat` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_real_time_chat`:
...
* Server: `clientVars`, `handleMessage`
* Client: `aceInitialized`, `handleClientMessage_CUSTOM`
