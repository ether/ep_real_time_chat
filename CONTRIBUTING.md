# Contributing to ep_real_time_chat

Thanks for helping improve `ep_real_time_chat` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_real_time_chat` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_real_time_chat from your local clone
pnpm run plugins i --path ../ep_real_time_chat

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_real_time_chat/` are picked up after a server restart.
...
* Etherpad version, ep_real_time_chat version, browser + OS
* Steps to reproduce
